-- Function: public.randompointsinpolygon(geometry, integer)

-- DROP FUNCTION public.randompointsinpolygon(geometry, integer);

CREATE OR REPLACE FUNCTION public.randompointsinpolygon(geom geometry, num_points integer)
...
BEGIN
  ...
  -- Modified from original function - this 
  -- doesn't try to check that the area in 
  -- which you want to put your points is 
  -- sensible. This deals with extremely small
  -- multi-part polygons and also with high-rises
  -- where there are lots of points in a very small
  -- geographic area.
  IF target_proportion < 0.000000001 THEN
...
END$BODY$
  ...
  COST 100
  ROWS 1000;
ALTER FUNCTION public.randompointsinpolygon(geometry, integer)
  OWNER TO postgres;
